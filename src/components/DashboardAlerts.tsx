...
import { AlertTriangle, AlertCircle, RotateCw, X } from "lucide-react";
        ...
        {alerts.map((a, idx) => {
          ...
          return (
            <button
              ...
              className="flex items-center w-full text-left bg-white/10 rounded-md p-3 transition hover:bg-white/20 space-x-3"
            >
              <Icon size={20} className={color} />
              <div>
                <span className="font-semibold text-white text-sm">{a.message}</span>
                {a.level && (
                  <span className="ml-2 rounded px-2 py-0.5 bg-red-600/80 text-xs text-white font-semibold">{a.level}</span>
                )}
              </div>
            </button>
          );
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-dark border border-white/20 rounded-xl p-6 min-w-[340px] max-w-sm relative">
            ...
            <div className="flex gap-2">
              <button
                className="bg-white/10 text-white px-3 py-1.5 rounded font-semibold text-xs"
                onClick={() => setModalIdx(null)}
              >
                Dismiss
              </button>
              <button
                className="bg-accent text-primary px-3 py-1.5 rounded font-semibold text-xs"
                onClick={() => setModalIdx(null)}
              >
                Mark as Resolved
              </button>
              <button
                className="bg-red-600 text-white px-3 py-1.5 rounded font-semibold text-xs"
                onClick={() => setModalIdx(null)}
              >
                Flag
              </button>
